...
- `test.html` 對它做**字面比對**（斷言頁首必須是 `0.6.2`），跳號時要一併改測試 —— 那正是「這是一件大事」的提醒。
...
| # | 檔案 | 位置 |
|---|---|---|
| 1 | `sw.js` | `CACHE_NAME = "life-calibration-v0.4.13"` 的尾碼 |
| 2 | `index.html` | `<link href="./style.css?v=0.4.13">` |
| 3 | `index.html` | `<script src="./app.js?v=0.4.13">` |
| 4 | `sw.js` | `APP_SHELL` 內的 `"./style.css?v=0.4.13"` |
| 5 | `sw.js` | `APP_SHELL` 內的 `"./app.js?v=0.4.13"` |
...
兩個號碼可以長期不一致（現在是功能 `0.6.2` 對資產 `0.4.13`），那不是錯誤，是設計。兩者各自遞增，誰大誰小沒有意義 —— 資產號跳得比功能號勤，因為純視覺改動也要跳它。
